#services\evaluation\bm25_evaluator.py
import json
import ir_datasets
import numpy as np
...
def evaluate_bm25_model(searcher: BM25Searcher, dataset_name: str, dataset_key : str ,results_path: str = None):
    if results_path is None:
            results_path = fr"C:\Users\muhammad\Desktop\ir\data\evaluation_results\{dataset_key}_bm25_evaluation_results.json"
    dataset = ir_datasets.load(dataset_name)
    qrels = load_qrels(dataset_name)
    precisions_at_10, recalls, reciprocal_ranks, average_precisions = [], [], [], []

    print("Evaluating BM25 model...")
